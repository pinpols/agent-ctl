...
- Produces:`Gateway._invoke_target(provider: Provider, target: Target, request: NormalizedRequest) -> tuple[NormalizedResponse, list[Attempt]]`;终态错误立即抛 `TerminalError`;可重试错误退避重试至 `max_attempts_per_target`,耗尽抛 `RetriableError`;每次尝试产 `Attempt`。`Gateway.__init__(router, providers: dict[str, Provider], cost_meter, store, cache=None, retry=RetryConfig(), cache_enabled=True, cache_ttl_s=600)`。
...
def test_retriable_then_success_within_target():
    ...
    resp, attempts = gw._invoke_target(FakeProvider(["retriable", "ok"]), T, REQ)
    assert resp.text == "fake-ok"
...
def test_terminal_not_retried():
    ...
    gw = _gw(p)
    with pytest.raises(TerminalError):
        gw._invoke_target(p, T, REQ)
    assert len(p.calls) == 1  # 终态不重试

def test_retriable_exhausted_raises():
...
class Gateway:
    ...
    ) -> tuple[NormalizedResponse, list[Attempt]]:
        attempts: list[Attempt] = []
...
def test_all_targets_fail_records_error(tmp_path):
    store = SqliteCaptureStore(str(tmp_path / "c.db"))
...
def test_cache_hit_skips_provider(tmp_path):
    ...
    assert store.list_recent(1)[0].cache_hit is True
...
# 追加方法到 Gateway 类
    def invoke(self, request: NormalizedRequest) -> NormalizedResponse:
        ...
        for idx, target in enumerate(targets):
            ...
            try:
                resp, attempts = self._invoke_target(provider, target, request)
                ...
                all_attempts.extend(self._tail_attempts(target, exc, "terminal"))
                self._capture(request, meta, started, model_resolved=target.name,
                ...
                all_attempts.extend(self._tail_attempts(target, exc, "retriable"))
                continue  # 回退下一目标
    ...
    def _tail_attempts(self, target, exc, outcome) -> list[Attempt]:
        # _invoke_target 已记录其内部尝试;此处仅在它把异常抛出后补登一条边界标记。
        return []
...
- Produces: `GatewayClient.from_config(config: Config, providers: dict[str, Provider]) -> GatewayClient`;`messages(model: str, messages: list[dict], max_tokens: int = 1024, temperature=None, tools=None, **metadata) -> NormalizedResponse`(metadata 透传 consumer/call_site/trace_id/prompt_version)。
...
class _FakeMessages:
    def __init__(self, behavior): self._b = behavior
...
- Produces: `main(argv: list[str] | None = None) -> int`;子命令 `captures --limit N`(打印近期记录)、`cost`(打印成本汇总)、`doctor`(检查配置:db 目录可写、路由非空、prod profile 下价表非空)。
...
- §2 范围五件事 → Tasks 3/9/10(路由/回退/重试)、5(成本)、8(缓存)、7+10(捕获);限流/eval/大盘/prompt 版本/语义缓存均按非目标未纳入。✓
- §3 CallRecord 全字段 → Task 2 定义 + Task 10 落库(prompt_version 经 metadata 预留)。✓
